...
parser =
    map Params <|
        map Internal (string </> s "channels" <?> Query.string "after" <?> Query.string "before" <?> Query.map parseState (Query.string "state"))
...
toString (Params internal) =
    absolute [ internal.spaceSlug, "channels" ] (buildQuery internal)
