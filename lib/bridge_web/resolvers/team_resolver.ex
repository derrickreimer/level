defmodule BridgeWeb.TeamResolver do
  ...
  def users(team, args, _info) do
    ...
    nodes = Repo.all(cursored_query)
    has_next_page = length(nodes) > limit
    has_previous_page =
      case Repo.one(from u in edge_query, limit: 1) do
        nil -> false
        node -> List.first(nodes).id != node.id
      end

    edges = Enum.map(Enum.take(nodes, limit), fn node ->
