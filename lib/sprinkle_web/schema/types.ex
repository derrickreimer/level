defmodule SprinkleWeb.Schema.Types do
  @moduledoc """
  GraphQL type definitions.
  """

  use Absinthe.Schema.Notation
  alias SprinkleWeb.Schema.Helpers

  import_types SprinkleWeb.Schema.Enums
  import_types SprinkleWeb.Schema.Scalars
  import_types SprinkleWeb.Schema.InputObjects
  import_types SprinkleWeb.Schema.Connections
  import_types SprinkleWeb.Schema.Mutations
  ...
  object :user do
    ...
    field :recipient_id, non_null(:id) do
      resolve fn user, _, _ ->
        {:ok, Sprinkle.Threads.get_recipient_id(user)}
      end
    end
    ...
    field :team, non_null(:team) do
      resolve fn user, _, _ ->
        batch({Helpers, :by_id, Sprinkle.Teams.Team}, user.team_id, fn batch_results ->
    ...
    field :drafts, non_null(:draft_connection) do
      ...
      resolve &SprinkleWeb.UserResolver.drafts/3
    end
  end

  @desc "A `Team` is the main organizational unit for a Sprinkle account."
  object :team do
    field :id, non_null(:id)
    ...
    field :users, non_null(:user_connection) do
      ...
      arg :after, :cursor
      arg :order_by, :user_order
      resolve &SprinkleWeb.TeamResolver.users/3
    end
  end
  ...
  object :draft do
    ...
    field :user, non_null(:user) do
      resolve fn draft, _, _ ->
        batch({Helpers, :by_id, Sprinkle.Teams.User}, draft.user_id, fn batch_results ->
    ...
    field :team, non_null(:team) do
      resolve fn draft, _, _ ->
        batch({Helpers, :by_id, Sprinkle.Teams.Team}, draft.team_id, fn batch_results ->
