defmodule NeuronWeb.Schema.Types do
  @moduledoc """
  GraphQL type definitions.
  """

  use Absinthe.Schema.Notation
  alias NeuronWeb.Schema.Helpers

  import_types NeuronWeb.Schema.Enums
  import_types NeuronWeb.Schema.Scalars
  import_types NeuronWeb.Schema.InputObjects
  import_types NeuronWeb.Schema.Connections
  import_types NeuronWeb.Schema.Mutations
  ...
  object :user do
    ...
    field :recipient_id, non_null(:id) do
      resolve fn user, _, _ ->
        {:ok, Neuron.Threads.get_recipient_id(user)}
      end
    end
    ...
    field :team, non_null(:team) do
      resolve fn user, _, _ ->
        batch({Helpers, :by_id, Neuron.Teams.Team}, user.team_id, fn batch_results ->
    ...
    field :drafts, non_null(:draft_connection) do
      ...
      resolve &NeuronWeb.UserResolver.drafts/3
    end
  end

  @desc "A `Team` is the main organizational unit for a Neuron account."
  object :team do
    field :id, non_null(:id)
    ...
    field :users, non_null(:user_connection) do
      ...
      arg :after, :cursor
      arg :order_by, :user_order
      resolve &NeuronWeb.TeamResolver.users/3
    end
  end
  ...
  object :draft do
    ...
    field :user, non_null(:user) do
      resolve fn draft, _, _ ->
        batch({Helpers, :by_id, Neuron.Teams.User}, draft.user_id, fn batch_results ->
    ...
    field :team, non_null(:team) do
      resolve fn draft, _, _ ->
        batch({Helpers, :by_id, Neuron.Teams.Team}, draft.team_id, fn batch_results ->
