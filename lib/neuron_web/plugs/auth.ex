defmodule NeuronWeb.Auth do
  ...
  alias Neuron.Repo
  alias Neuron.Teams
  alias NeuronWeb.Router.Helpers
  alias NeuronWeb.UrlHelpers
  ...
  """
  def jwt_secret do
    Application.get_env(:neuron, NeuronWeb.Endpoint)[:secret_key_base]
  end
      ...
      ["Bearer " <> token] ->
        case verify_signed_jwt(token) do
          %Joken.Token{claims: %{"sub" => user_id}, error: nil} ->
            user = Repo.get(Neuron.Teams.User, user_id)
