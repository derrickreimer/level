defmodule BridgeWeb.SubdomainTest do
  use BridgeWeb.ConnCase
  alias BridgeWeb.Subdomain
  ...
  describe "validate_host/2" do
    ...
    test "returns the conn if host matches config", %{conn: conn} do
      conn =
        conn
          |> Map.put(:host, Application.get_env(:bridge, BridgeWeb.Endpoint)[:url][:host])
  ...
  describe "extract_subdomain/2" do
    setup %{conn: conn} do
      root_domain = Application.get_env(:bridge, BridgeWeb.Endpoint)[:url][:host]
