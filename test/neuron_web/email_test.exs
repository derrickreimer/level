defmodule NeuronWeb.EmailTest do
  use ExUnit.Case
  use Bamboo.Test

  describe "invitation_email/1" do
    setup do
      team = %Neuron.Teams.Team{name: "Acme", slug: "acme"}
      invitor = %Neuron.Teams.User{email: "derrick@acme.com"}

      invitation = %Neuron.Teams.Invitation{
        team: team,
        invitor: invitor,
        email: "bob@acme.com",
        token: "xxx"
      ...
      email = NeuronWeb.Email.invitation_email(invitation)
    ...
    test "includes the invitation url", %{email: email} do
      assert email.html_body =~ "http://acme.neuron.test:4001/invitations/xxx"
      assert email.text_body =~ "http://acme.neuron.test:4001/invitations/xxx"
    end

    test "sends from support", %{email: email} do
      assert email.from == {"Neuron", "support@neuron.test"}
    end
  end
end
