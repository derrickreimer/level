defmodule NeuronWeb.SubdomainTest do
  use NeuronWeb.ConnCase
  alias NeuronWeb.Subdomain
  ...
  describe "validate_host/2" do
    ...
    test "raises an exception if host does not match config", %{conn: conn} do
      assert_raise RuntimeError, fn ->
        conn
        |> Map.put(:host, "neurontonowhere.test")
        |> Subdomain.validate_host
      end
    end
    ...
    test "returns the conn if host matches config", %{conn: conn} do
      conn =
        conn
          |> Map.put(:host, Application.get_env(:neuron, NeuronWeb.Endpoint)[:url][:host])
  ...
  describe "extract_subdomain/2" do
    setup %{conn: conn} do
      root_domain = Application.get_env(:neuron, NeuronWeb.Endpoint)[:url][:host]
