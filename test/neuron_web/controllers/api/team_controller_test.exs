defmodule NeuronWeb.API.TeamControllerTest do
  use NeuronWeb.ConnCase

  alias NeuronWeb.API.TeamView
  ...
  describe "POST /api/teams with valid data" do
    ...
    test "creates new team", %{params: %{slug: slug}} do
      assert Repo.get_by!(Neuron.Teams.Team, %{slug: slug})
    end

    test "creates new user as the owner of the team",
      %{params: %{slug: slug, email: email}} do

      user = Repo.get_by!(Neuron.Teams.User, %{email: email})
  ...
  describe "POST /api/teams with invalid data" do
    setup %{conn: conn} do
      ...
      user_count = count_all(Neuron.Teams.User)
      team_count = count_all(Neuron.Teams.Team)
    ...
    test "does not not create a new team", %{team_count: team_count} do
      assert team_count == count_all(Neuron.Teams.Team)
    end

    test "does not not create a new user", %{user_count: user_count} do
      assert user_count == count_all(Neuron.Teams.User)
    end
