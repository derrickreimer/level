defmodule Level.GroupsTest do
  ...
  describe "get_group/2" do
    ...
    test "returns the group if it's private and user is a member", %{
      ...
      Groups.grant_private_group_access(user, group, another_space_user)
  ...
  describe "grant_private_group_access/3" do
    setup do
    ...
    test "does not change state for subscribed users", %{
      ...
      assert Groups.get_user_state(group, space_user) == :subscribed
      Groups.grant_private_group_access(user, group, space_user)
      assert Groups.get_user_state(group, space_user) == :subscribed
    end
  end
  ...
  describe "revoke_private_group_access/2" do
    setup do
      {:ok, %{space_user: space_user} = result} = create_user_and_space()
      {:ok, %{group: group}} = create_group(space_user, %{is_private: true})
      {:ok, Map.merge(result, %{group: group})}
    ...
    test "revokes the user's access to private groups", %{
      ...
      Groups.grant_private_group_access(user, group, another_user)
      Groups.subscribe(group, another_user)
      Groups.revoke_private_group_access(user, group, another_user)
  ...
  describe "can_privatize?/1" do
    test "is true if user has private access" do
      assert {:ok, true} = Groups.can_privatize?(%GroupUser{access: "PRIVATE"})
    end

    test "is false if user only has public access" do
      assert {:ok, false} = Groups.can_privatize?(%GroupUser{access: "PUBLIC"})
    end
  ...
  describe "can_publicize?/1" do
    test "is true if user has private access" do
      assert {:ok, true} = Groups.can_publicize?(%GroupUser{access: "PRIVATE"})
    end

    test "is false if user only has public access" do
      assert {:ok, false} = Groups.can_publicize?(%GroupUser{access: "PUBLIC"})
    end
